...
class VideoDataset(data.Dataset):
    ...
    def __getitem__(self,index):
        ...
        target = cv2.cvtColor(self.load_img(env_hr,key,self.resolutions_hr[index]),cv2.COLOR_GRAY2RGB).transpose((2,0,1))
        input = cv2.cvtColor(self.load_img(env_lr,key,self.resolutions_lr[index]),cv2.COLOR_GRAY2RGB).transpose((2,0,1))
        inputs = []
        ...
        for i in seq:
            ...
            if key1 in self.keys_lr:
                ...
                input1 = cv2.cvtColor(input1,cv2.COLOR_GRAY2RGB).transpose((2,0,1))
